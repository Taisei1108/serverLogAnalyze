...
#サーバーの種類を確認する、空の辞書になかったら空リストを登録 &　サブネットマスクごと
serve_list = {}
for i in serve_log['num']:
    if serve_log['network'][i][-2:] not in serve_list.keys():
        serve_list[str(serve_log['network'][i][-2:])] = [] 

#辞書にサーバーの種類ごとに識別番号'num'を登録　＆　サブネットマスクごと
for i in serve_log['num']:
    serve_list[str(serve_log['network'][i][-2:])].append(serve_log['num'][i])
